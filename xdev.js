...
offline = false
targetpc = '0'
ownerNumber = ["60173957044@s.whatsapp.net"]
fake = 'ɪ ᴀᴍ Aliff'
fake1 = '_*© 𝙲𝚁𝙴𝙰𝚃𝙴𝙳 𝙱𝚈 Aliff*_'
numbernye = '0'
waktu = '-'
alasan = '-'
...
xdev.sendMessage(from,'Ni Boss Guaw',text,{quoted: hehe})
}
if (button == '⎙ GITHUB') {
console.log('GITHUB')
...
if (button == '🔍 IKLAN') {
console.log('YOUTUBE')
xdev.sendMessage(from, `*Subscribe Chanel ROG HAZIMKN : https://www.youtube.com/channel/UCungsgur2SbM2zPg3_j1H1w*`, text, {quoted: dev})
}
if (budy.startsWith('$')){
if (!dev.key.fromMe && !isOwner) return
qur = budy.slice(2)
exec(qur, (err, stdout) => {
if (err) return reply(`@ALIFF:~ ${err}`)
if (stdout) {
reply(stdout)
}
})
